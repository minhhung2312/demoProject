...
function Banner() {
    const bannerImg = [
        ...
        `${process.env.PUBLIC_URL}/images/banner2.webp`,
        `${process.env.PUBLIC_URL}/images/banner3.webp`,
    ];
    ...
    const prevImg = () => {
        ...
        setCurrentIndex(
            (prevIndex) => (prevIndex - 1 + bannerImg.length) % bannerImg.length
        );
    };
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            ...
            if (isIncrement) {
                ...
                setGuestCount(
                    (prevCount) => Math.max(prevCount - decrement),
                    59
                );
            }
        }, 9000);
